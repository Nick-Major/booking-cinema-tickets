...
class CSSConflictFinder {
  ...
  extractRules(cssContent) {
    ...
    const ruleBlocks = withoutComments
      .split("}")
  ...
  findConflicts(rules, filePath) {
    ...
    selectorMap.forEach((occurrences, selector) => {
      if (occurrences.length > 1) {
        ...
        if (
          uniqueFiles.size === 1 &&
          occurrences[0].file.includes("normalize.css")
        ) {
          return
        }
  ...
  generateReport() {
    const report = {
      summary: {
        ...
        highPriority: this.conflicts.filter((c) => c.severity === "HIGH")
          .length,
        mediumPriority: this.conflicts.filter((c) => c.severity === "MEDIUM")
          .length,
    ...
    // ОБНОВЛЯЕМ ПУТИ ДЛЯ ОТЧЕТОВ:
    const reportsDir = path.join(__dirname, "../../reports/css-analysis")
    if (!fs.existsSync(reportsDir)) {
      fs.mkdirSync(reportsDir, { recursive: true })
    }

    const jsonReportPath = path.join(reportsDir, "css-conflict-report.json")
    const htmlReportPath = path.join(reportsDir, "css-conflict-report.html")

    fs.writeFileSync(jsonReportPath, JSON.stringify(report, null, 2))
    this.generateHTMLReport(report, htmlReportPath)

    this.printSummary(report, htmlReportPath, jsonReportPath)
  }
...
<body>
    ...
    <div class="summary">
        <h2>📊 Статистика</h2>
        <p><span class="high">🔴 Высокий приоритет: ${
          report.summary.highPriority
        }</span></p>
        <p><span class="medium">🟡 Средний приоритет: ${
          report.summary.mediumPriority
        }</span></p>
        <p><span class="low">🟢 Низкий приоритет: ${
          report.summary.lowPriority
        }</span></p>
        <p><strong>📊 Всего конфликтов: ${
          report.summary.totalConflicts
        }</strong></p>
        <p><strong>📊 Всего дубликатов: ${
    ...
    ${report.conflicts
      .map(
        (conflict) => `
        <div class="conflict ${
          conflict.severity === "HIGH" ? "high-severity" : "medium-severity"
        }">
            <h3>Селектор: ${conflict.selector}</h3>
            <p><strong>Тип:</strong> ${
              conflict.type
            } | <strong>Приоритет:</strong> ${conflict.severity}</p>
    ...
    ${report.duplicates
      .map(
        (duplicate) => `
        <div class="conflict ${
          duplicate.severity === "HIGH" ? "high-severity" : "low-severity"
        }">
            <h3>Свойство: ${duplicate.key}</h3>
            <p><strong>Тип:</strong> ${
              duplicate.type
            } | <strong>Приоритет:</strong> ${duplicate.severity}</p>
  ...
  // ОБНОВЛЯЕМ ВЫВОД ПУТЕЙ:
  printSummary(report, htmlReportPath, jsonReportPath) {
    ...
    if (report.conflicts.length > 0) {
      ...
      report.conflicts
        ...
        .forEach((conflict, index) => {
          ...
          console.log(
            `   Файлов: ${
              new Set(conflict.occurrences.map((o) => o.file)).size
            }`
          )
    ...
    console.log("\n📁 Отчеты сохранены:")
    console.log(`   - ${jsonReportPath} (машиночитаемый)`)
    console.log(`   - ${htmlReportPath} (человекочитаемый)`)
  }
}
